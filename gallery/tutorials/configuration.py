...
#
# By default, the ``log_dir`` is a directory called ``logs`` within current working directory from which ASPIRE was invoked, but
# it can be changed to an absolute path (for instance, to go to the same folder as a custom configuration file stored elsewhere):
#
#    .. code-block:: yaml
#    [logging]
#    log_dir=/tmp/my_proj
#
# ASPIRE directs output streams to both the console and to a log file generated for each session. The logging verbosity for both of these
# can be specified individually via ``console_level`` and ``log_file_level``. These levels are passed through to Python's logging system, so the
# labels are the same: ``DEBUG, INFO, WARNING, ERROR, CRITICAL``. In general, it may make sense to have the log file store more verbose output
# than the console. By default, only ``INFO`` and above level messages are printed to the terminal, while detailed ``DEBUG`` messages are saved to disk:
#
#    .. code-block:: yaml
#    [logging]
#    console_level: INFO
...
#    log_file_level: DEBUG
#
# By default, the filename of the log file is ``aspire-{MM-DD-YYYYThh:mm:ss}.log``. This can be customized via the ``log_file_name`` option. Note that
# if the same log file is used for multiple sessions, it will be appended to, not overwritten.
...
#
# This file exposes options that ASPIRE's ``config.yaml`` does not, for example the precise formatting options for log messages. However, values from ASPIRE's configuration
# are passed through this file, so caution is needed when modifying it.
#
# More details:
