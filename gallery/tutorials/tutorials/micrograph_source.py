...
micrograph = MicrographSource(
    sim, particles_per_micrograph=4, micrograph_size=500, micrograph_count=2, seed=1234
)
...
corrupted_sim = Simulation(
    L=64, n=2 * 4, seed=1234, C=1, amplitudes=1, offsets=0, unique_filters=ctfs
)
...
colliding_sim = Simulation(
    L=64, n=2 * 20, seed=1234, C=1, amplitudes=1, offsets=0, unique_filters=ctfs
)
...
inbound_micrograph = MicrographSource(
    colliding_sim,
    boundary=20,
    interparticle_distance=1,
    noise_adder=noise,
...
micrograph_id, local_particle_id = colliding_micrograph.get_micrograph(
    global_particle_id
)
...
p_square = colliding_micrograph.clean_micrographs[micrograph_id].asnumpy()[0][
...
simulation_particle_id = colliding_micrograph.get_particle(
    micrograph_id, local_particle_id
)
