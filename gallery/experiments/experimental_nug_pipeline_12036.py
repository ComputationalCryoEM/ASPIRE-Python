...
This pipeline begins with 93 class averages that were generated (as described
in the publication) from the EMPIAR 12036 picked particle data, available here:

https://www.ebi.ac.uk/empiar/EMPIAR-12036
...
EMPIAR-12036 associated reconstruction found here:

https://www.ebi.ac.uk/emdb/EMD-14803
"""
...
oriented_fn = "nug_12036_oriented.star"
volume_output_fn = "nug_12036_abinitio.mrc"
...
# ----------------
#
# The 93 class averages can be accessed as an ASPIRE ``Image`` object
# via the built-in downloader utility.
logger.info("Load Precomputed Class Averages")
ims = nug_12036()
avgs = ArrayImageSource(ims, symmetry_group="D3")
...
# we use the 5 iterations of proximal refinement and disable searching over
# the commonline shift space.

logger.info("Begin Orientation Estimation")
orient_est = CommonlineNUG(avgs, max_shift=0, pr_iters=5)
...
# this is a D3 symmetric molecule the ``symmetry_group`` source attribute
# will flow through the pipeline to ``oriented_src``. Then each image will be
# repeated for each of the 5 additional symmetry induced rotations during
# back-projection. In general, this boosts the effective number of images used in
# the reconstruction from ``n_imgs`` to ``6 * n_imgs``, or from 93 to 558 in this case.
