...
class Cov2DTestCase(TestCase):
    ...
    def testGetMean(self):
        ...
        mean_coeff = self.cov2d._get_mean(self.coeff_clean.asnumpy())
    ...
    def testGetCovar(self):
        ...
        covar_coeff = self.cov2d._get_covar(self.coeff_clean.asnumpy())
...
class Cov2DTestCaseCTF(Cov2DTestCase):
    ...
    @property
    def h_ctf_fb(self):
        return [self.basis.filter_to_basis_mat(f) for f in self.unique_filters]
    ...
    def testGetCWFCoeffsCTFargs(self):
        ...
        with raises(RuntimeError, match=r".*Given ctf_basis.*"):
