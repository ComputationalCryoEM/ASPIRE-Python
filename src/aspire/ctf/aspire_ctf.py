...
class CtfEstimator:
    ...
    def __init__(self, pixel_size, cs, amplitude_contrast, voltage, psd_size, num_tapers):
    ...
    def ctf_preprocess(self, micrograph, block_size):
        ...
        block_list = [micrograph[i * step_size:(i + 2) * step_size, j * step_size:(j + 2) * step_size] for i in
                      range(range_y) for j in range(range_x)]
        ...
        block = block - (block_sum / (512 ** 2))  # equals to the matlab version (11-7-19)
    ...
    def ctf_tapers(self, N, R, L):
        ...
        data_tapers = data_tapers.copy()  # absolute difference from matlab: 10^-15, relative: 10^-11
    ...
    def ctf_estimate_psd(self, blocks, tapers_1d, num_1d_tapers):
        ...
        fft_class_f = pyfftw.FFTW(blocks_mt_pre_fft, blocks_mt_post_fft, axes=(-2, -1), direction='FFTW_FORWARD')
        ...
        for x in range(num_1d_tapers ** 2):
            np.matmul(np.reshape(tapers_1d[:, x // num_1d_tapers], (tapers_1d.shape[0], 1)),
                      np.reshape(tapers_1d[:, x % num_1d_tapers], (1, tapers_1d.shape[0])), out=taper_2d)
        ...
        thon_rings = np.fft.fftshift(blocks_mt)  # max difference 10^-13, max relative difference 10^-14
    ...
    def ctf_opt1d(self, thon_rings, pixel_size, cs, lmbd, w, N):
        ...
        center = N // 2
        [X, Y] = np.meshgrid(np.arange(0 - center, N - center) / N, np.arange(0 - center, N - center) / N)
        ...
        for f in range(500, 10000):
            ctf_im = np.abs(
                np.sin(np.pi * lmbd * f * r_ctf_sq - 0.5 * np.pi * (lmbd ** 3) * cs * (10 ** 6) * np.square(r_ctf_sq) + w))
            ...
            c[f - 500, :] = np.divide(np.sum(np.multiply(ctf_im, signal), axis=0), (np.multiply(Sx, Sy)))
    ...
    def ctf_gd(self, signal, df1, df2, angle_ast, r, theta, pixel_size, g_min, g_max, amplitude_contrast, lmbd, cs):
        ...
        while np.maximum(np.maximum(dx,dy),dz)>stop_cond:
            ...
            dy_c1 = np.sum(np.multiply(np.multiply(np.multiply(np.multiply(np.sign(outer_sine), outer_cosine), a), np.cos(2 * theta)), signal))
            dz_c1 = np.sum(np.multiply(np.multiply(np.multiply(np.multiply(np.sign(outer_sine), outer_cosine), a), np.sin(2 * theta)), signal))
