...
class FFBBasis2D(FBBasis2D):
    ...
    def calculate_bispectrum(self, coef):
        """
        ...
        # Mainly due to memory concerns, this function expects a coef set (single image).
        assert np.size(coef.shape) == self.count
        ...
        for ind1 in tqdm(range(self.complex_count)):
            ...
            for ind2 in range(self.complex_count):
                ...
                k3 = k1 + k2
                intermodulated_coef_inds = np.where(angular_indices == k3)
                # print(intermodulated_coef_inds)
                if intermodulated_coef_inds:
                    Q3 = radial_indices[intermodulated_coef_inds]
                    # print(Q3)

                    # Coef3 = coef[intermodulated_coef_inds]
                    # B[ind1, ind2, intermodulated_coef_inds] = coef1 * coef2 * Coef3
