...
def besselj_zeros(nu, k):
    ...
    while n < k:
        ...
        assert             check_besselj_zeros(nu,  z[n - 2 : n + j]),            "Unable to properly estimate Bessel function zeros."        
...
def unique_coords_nd(N, ndim, shifted=False, normalized=True, dtype=np.float32):
    ...
    """
    assert         ndim in (2,  3), "Only two- or three-dimensional basis functions are supported."    
