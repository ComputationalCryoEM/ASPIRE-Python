function [clstack,corrstack,shift_equations,shift_equations_map,clstack_mask]=...
    cryo_clmatrix_cpu_fast_v1(pf,NK,verbose,max_shift,shift_step,map_filter_radius,...
...
rk2=rk(1:rmax);
all_shift_phases = zeros(rmax, n_shifts);
for shiftidx=1:n_shifts
    shift=-max_shift+(shiftidx-1)*shift_step;
    all_shift_phases(:, shiftidx)=exp(-2*pi*sqrt(-1).*rk2.*shift./(2*rmax+1));
end
...
for k1=1:n_proj
    ...
    all_P1_shifted = struct();
    all_P1_shifted_flipped = struct();
    
    for shiftidx=1:n_shifts
        tmp=bsxfun(@times,P1,all_shift_phases(:, shiftidx));
        all_P1_shifted(shiftidx).x=tmp';
        tmp=bsxfun(@times,P1_flipped,all_shift_phases(:, shiftidx));
        all_P1_shifted_flipped(shiftidx).x=tmp';
    end
    ...
    for k2=subsetK2
        ...
        for shiftidx=1:n_shifts     
            %P1_shifted=all_P1_shifted(:, :, shiftidx);
            %P1_shifted_flipped=all_P1_shifted_flipped(:, :, shiftidx);
            
            % Compute correlations in the positive r direction           
            C1=real(all_P1_shifted(shiftidx).x*P2);
            [sval,sidx]=max(C1(:));
            
            % Compute correlations in the negative r direction
            C1=real(all_P1_shifted_flipped(shiftidx).x*P2);
            [sval_2,sidx_2]=max(C1(:));
            ...
            if sval>corrstack(k1,k2)
                ...
                clstack(k1,k2)=cl1;
                clstack(k2,k1)=cl2 + cl2_addition;
